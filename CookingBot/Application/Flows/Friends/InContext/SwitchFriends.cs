...
{
	private static (string addFriend, string checkRecipes) Buttons = ("Добавить друга", "Посмотреть рецепты");
	...
	protected override async Task Handle(Update update, DetailContext<BasePayload, FriendsContext> context)
	{
		...
		if (update.Message?.Text == Buttons.checkRecipes)
		{
			context.State.GoTo(FriendsContext.CheckRecipe);
			return;
		}
	}
